...
function renderHikes(list) {
  ...
  if (list.length === 0) {
    container.innerHTML = `<p class="no-results">No hikes found matching your search.</p>`;
    return;
  }

  container.innerHTML = list.map(hike => `
    <div class="hike-card">
      <div class="hike-content">
        ...
        <p>Difficulty: 
          <span class="rating" role="img" aria-label="${hike.difficulty} out of 5">
            ${"🥾".repeat(hike.difficulty)}${"▫️".repeat(5 - hike.difficulty)}
          </span>
        </p>
      </div>
    </div>
